<!DOCTYPE html>
<html>
  <head>
    <title></title>
    <meta name="description" content=""/>
    <?php include_once __DIR__ . '../../php/head.inc.php' ?>
  </head>

  <body>
    <!-- header -->
    <?php include_once __DIR__ . '../../php/header.inc.php' ?>

    <main id="" class="page-content">
      <p style="width: 100%; text-align: center; margin-top: 50px; font-size: 25px; background: #a00; padding: 25px 0; color: #fff;">Er is een fout opgetreden bij het opvragen van de pagina!</p>
    </main>

    <!-- footer -->
    <?php include_once __DIR__ . '../../php/footer.inc.php' ?>

    <!-- scripts -->
    <?php include_once __DIR__ . '../../php/js.inc.php' ?>
  </body>
</html>
